...
function TaskScheduler() {
  ...
  self.scheduleTask = function (id, cronSchedule, worker) {
    ...
    return self._findTask(id).then(function (task) {
      const interval = cronParser.parseExpression(cronSchedule, {tz: self.timezone});
      ...
      if (!task) {
        ...
        if (task.cronSchedule !== cronSchedule || (!task.receivedTime && task.nextScheduledTime > nextTime)) {
          ...
          console.log(
            "next scheduled time for task '" + task._id + "' is " + _getFormattedTime(task.nextScheduledTime)
          );
        }
      }
    });
  ...
  self._poll = function () {
    return self
      ...
      .then(function (task) {
        ...
        return Promise.resolve(self._processTask(task))
  ...
  self._processTask = function (task) {
    const worker = self._taskWorkers[task._id];
    if (!worker) {
      return Promise.reject(new Error('No worker registered for scheduled task with id: ' + task._id));
    }
  ...
  self._receiveTask = function () {
    ...
    return self._getMongoCollection()
      .then(function (collection) {
        return collection.findOneAndUpdate(query, update, {returnDocument: 'after'});
      })
  ...
  self._findTask = function (id) {
    return self._getMongoCollection().then(function (collection) {
  ...
  self._updateTask = function (task, update) {
    return self
      ._getMongoCollection()
      .then(function (collection) {
        return Promise.all([collection, collection.updateOne({_id: task._id}, update)]);
      })
      .then(function ([collection]) {
  ...
  self._getMongoCollection = function () {
    ...
    return self.databasePromise().then(function (db) {
